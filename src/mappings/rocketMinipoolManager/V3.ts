...
import { rocketMinipoolDelegateV1, rocketMinipoolDelegateV2, rocketMinipoolDelegateV3 } from '../../../generated/templates'

/**
 * Occurs when a node operator makes an ETH deposit on his node to create a minipool.
 */
export function handleMinipoolCreatedV3(event: MinipoolCreated): void {
  ...
  } else if (event.block.number < BigInt.fromI32(5747646)) {
    rocketMinipoolDelegateV2.create(Address.fromString(minipool.id));
  } else {
    rocketMinipoolDelegateV3.create(Address.fromString(minipool.id))
  }
}
 ...
 */
export function handleMinipoolDestroyedV3(event: MinipoolDestroyed): void {
  // Preliminary null checks.
  if (
    event === null ||
    event.params === null ||
 ...
 */
export function handleIncrementNodeFinalisedMinipoolCountV3(
